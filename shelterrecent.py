...
if action and booking_id:
    try:
        ...
        user_query = """SELECT u.email, u.name,b.day FROM usereg u JOIN bookings b ON u.id = b.id WHERE b.bookingid = %s"""
        ...
        if user_result:
            ...
            {f"Congratulations! Your adoption request has been approved on {recipient_date}. Please wait for further communication from the shelter." if action == "accept" else f" We regret to inform you that your adoption request has been declined on {recipient_date}. We appreciate your interest in pet adoption and encourage you to consider other pets available for adoption."}
            ...
            try:
                ...
                # Update product_info table status to adopted
                product_update_query = """
                    UPDATE product_info p 
                    JOIN bookings b ON p.product_id = b.product_id 
                    SET p.status = 'adopted' 
                    WHERE b.bookingid = %s
                """
                cur.execute(product_update_query, (booking_id,))
                con.commit()
                ...
                <script>
                    ...
                    window.location.href = window.location.href.split('?')[0] + '?shelter_id=' + '{shelter_id}';
                </script>
                """)
    ...
    <div class="container">
        ...
        <div class="nav-item dropdown">
            ...
            <ul class="dropdown-menu" aria-labelledby="ordersDropdown">
                ...
                <li><a class="dropdown-item" href="usercompleted.py?shelter_id=%s"><i class="fas fa-check-circle"></i> Approved</a></li>
            </ul>
        </div>
    </div>
</nav>
...
if results:
    ...
    for row in results:
        ...
        img_path = f"./images/{profile_pic}" if profile_pic else "./images/default_pet.jpg"
        print(f"""
                    <tr>
                        <td>{sno}</td>
                        ...
                        <td><span class="status-badge status-adopted">Requested</span></td>
                        <td>
                        ...
                        <td>
                            <a href="?action=accept&booking_id={booking_id}&shelter_id={shelter_id}" class="btn-accept">
                                <i class="fas fa-check"></i> Accept
                            </a>
                            <a href="?action=reject&booking_id={booking_id}&shelter_id={shelter_id}" class="btn-reject">
...
print("""
    ...
    <script>
        function showUserDetails(userData) {
            ...
            const profilePicPath = userData.profile_pic ? `./images/${userData.profile_pic}` : './images/default_pet.jpg';
            const proofDocPath = userData.proof_attachment ? `./images/${userData.proof_attachment}` : '';
            
            modalContent.innerHTML = `
                <div class="user-detail-row">
                    <div class="user-detail-label">Profile Picture:</div>
                    <div><img src="${profilePicPath}" alt="Profile Picture" style="max-width: 200px; max-height: 200px; border-radius: 10px;"></div>
                </div>
                ${userData.proof_attachment ? `
                <div class="user-detail-row">
                    <div class="user-detail-label">Proof Document:</div>
                    <div><img src="${proofDocPath}" alt="Proof Document" style="max-width: 200px; max-height: 200px; border-radius: 10px;"></div>
                </div>
                ` : ''}
                <div class="user-detail-row">
