...
# Select data from shelter_info table with product count
cur.execute("""
    SELECT s.shelter_id, s.ownername, s.street, s.city, s.pin, s.state, s.mob, s.email, s.profile_pic, s.proof_attachment,
           COUNT(p.shelter_id) as shelter_count, s.registeredtime
    FROM shelter_info s 
    LEFT JOIN product_info p ON s.shelter_id = p.shelter_id 
    WHERE s.status='approved' 
    GROUP BY s.shelter_id, s.ownername, s.street, s.city, s.pin, s.state, s.mob, s.email, s.profile_pic, s.proof_attachment, s.registeredtime
""")
...
for index, shelter in enumerate(shelter_data):
    ...
    registered_date = shelter[11].strftime('%d-%m-%y') if shelter[11] else 'N/A'
    print(f"""
                    <tr>
                        <td><img src="./images/{shelter[8]}" class="shelter-profile" alt="Profile"></td>
                        <td>{shelter[1]}</td>
                        <td>
                            <i class="fas fa-phone"></i> {shelter[6]}<br>
                            <i class="fas fa-envelope"></i> {shelter[7]}
                        </td>
                        <td>
                            {shelter[2]}, {shelter[3]}<br>
                            {shelter[4]}, {shelter[5]}
                        </td>
                        <td>Active</td>
                        ...
                        <td>
                            <button type="button" class="btn btn-view" data-bs-toggle="modal" data-bs-target="#shelterModal{index}">
                                <i class="fas fa-eye"></i> Shelter info
                            </button>
                        </td>
                    </tr>
    """)
...
for index, shelter in enumerate(shelter_data):
    # Get product details for this shelter
    cur.execute("""
        SELECT Petname, Species, Breed, gender, Age, description, Animalimg
        FROM product_info
        WHERE shelter_id = %s
    """, (shelter[0],))
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <img src="./images/{shelter[8]}" class="shelter-modal-profile" alt="Profile">
                    <div class="row mt-3">
                        <div class="col-md-6">
                            <p><span class="shelter-info-label">Owner Name:</span> {shelter[1]}</p>
                            <p><span class="shelter-info-label">Mobile:</span> {shelter[6]}</p>
                            <p><span class="shelter-info-label">Email:</span> {shelter[7]}</p>
                        </div>
                        <div class="col-md-6">
                            <p><span class="shelter-info-label">Address:</span> {shelter[2]}</p>
                            <p><span class="shelter-info-label">City:</span> {shelter[3]}</p>
                            <p><span class="shelter-info-label">PIN:</span> {shelter[4]}</p>
                            <p><span class="shelter-info-label">State:</span> {shelter[5]}</p>
                        </div>
                    </div>
                    <div class="mt-3">
                        <p class="shelter-info-label">Proof Document:</p>
                        <img src="./images/{shelter[9]}" class="shelter-proof-img" alt="Proof Document">
                    </div>
                    <div class="mt-4">
                        <h4 class="shelter-info-label">Shelter Pets</h4>
                        <div class="row">
    """)
    ...
    for product in products:
        print(f"""
                            <div class="col-md-4 mb-3">
                                <div class="card">
                                    <img src="./images/{product[6]}" class="card-img-top" alt="{product[0]}" style="height: 150px; object-fit: cover;">
                                    <div class="card-body">
