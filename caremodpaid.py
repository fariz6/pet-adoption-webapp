...
SELECT DISTINCT
    ...
    cr.careresourcetype,
    p.serviceprice as payment_amount,
    p.paytime,
    p.paymethod
FROM 
    carebookings cb
...
WHERE 
    cb.status = 'paid' AND cr.centreid = %s
ORDER BY 
    cb.carebookingid
"""
...
<body>
    ...
    <section id="shelter-management" class="py-5">
        <div class="container main-container">
            <h2 class="section-title">Bookings Completed</h2>
...
for booking in bookings:
    ...
    status = booking[6]
    caretype = booking[7]
    bookingtime = booking[8]
    
    # Format bookingtime to dd-mm-yy
    if bookingtime:
        bookingtime = bookingtime.strftime('%d-%m-%y')
    
    center_name = booking[9]
    center_type = booking[10]
    address = booking[11]
    city = booking[12]
    state = booking[13]
    center_mob = booking[14]
    center_mail = booking[15]
    description = booking[16]
    image = booking[17]
    
    user_email = booking[18]
    username = booking[19]
    user_mob = booking[20]
    ...
    careresourcername = booking[23]
    careresourcetype = booking[24]
    payment_amount = booking[25]
    paytime = booking[26]
    paymentmode = booking[27]
    
    # Format paytime to dd-mm-yy HH:MM
    if paytime:
        paytime = paytime.strftime('%d-%m-%y %H:%M')

    # Status class will always be "primary" since all are completed
    status_class = "primary"

    # Determine service type badge class
    service_class = "info"
    if careresourcetype == "Training":
        service_class = "warning"
    elif careresourcetype == "Pet Spa":
        service_class = "success"
    elif careresourcetype == "Veterinary" or careresourcetype == "Veterinary Clinic":
    ...
    print(f"""
                                <tr>
                                    ...
                                    <td>{date}</td>
                                    <td>{paytime if paytime is not None else 'N/A'}</td>
                                    <td><span class="status-badge">Completed</span></td>
                                    <td>₹{payment_amount if payment_amount is not None else 'N/A'}</td>
                                    <td>{paymentmode if paymentmode is not None else 'N/A'}</td>
                                </tr>
    """)
    sno += 1
...
for booking in bookings:
    ...
    center_mail = booking[15]
    description = booking[16]
    image = booking[17] if booking[17] else "default.jpg"
    careresourcername = booking[23]
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <p><strong>Name:</strong> {careresourcername}</p>
                            <p><strong>Center Type:</strong> {center_type}</p>
                        </div>
                        <div class="col-md-6">
                            <p><strong>Location:</strong> {city}, {state}</p>
                            <p><strong>Email:</strong> {center_mail}</p>
                        </div>
                    </div>
                    <div class="mt-3">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <p><strong>Phone:</strong> {user_mob}</p>
